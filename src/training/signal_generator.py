            ...
            for s in samples:
                f.write(
                    json.dumps(
                        {
                            "query": s.query,
                            "response": getattr(s, "response", getattr(s, "answer", "")),
                            "answer": getattr(s, "answer", getattr(s, "response", "")),
                            "chs": s.chs,
                            "scr": getattr(s, "scr", 0.0),
                            "tve": getattr(s, "tve", 0.0),
                            "cr": getattr(s, "cr", 0.0),
                            "cdee": getattr(s, "cdee", 0.0),
                            "prompt": s.prompt,
                        }
                    )
                    + "\n"
